...
void GroupeDeComposants::centrerH(const Composant *reference, float espacement, bool appliquerNiveau, float niveau)
{
    if(m_composants.isEmpty())
        return;
    ...
    if(appliquerNiveau)
    {
        for(int i = 0; i < nb; i++) // mettre au même niveau
        {
            (*iteratorAt(i))->setY(niveau);
        }
    }
}
...
float GroupeDeComposants::longueur()const
{
    ...
    {
        somme += m_composants.at(i)->longueur();
    }

    somme += m_espacement * (m_composants.count() - 1);
