...
public:
    NCReportLabelData()
     : alignment(Qt::AlignLeft | Qt::AlignTop), wordbreak(false),
       rotation(0), hideRepeated(false), hasTranslation(false), hasCondFormat(false)
    {}
    ...
    class TextStyle {
    public:
        TextStyle() : forecolor(Qt::black), backcolor(Qt::white), font(QFont()), bgMode(0), bgDesign(false) {}
        QColor forecolor;
        QColor backcolor;
        QFont font;
        uint bgMode;
    ...
    QString displayValue, lastDisplayValue;
    QString lastValue;
...
public:
    ...
    { return ((NCReportLabelData*)d)->alignment; }

    inline void setAlignment( Qt::Alignment al )
    { ((NCReportLabelData*)d)->alignment = al; }
    ...
    virtual void readSubElements(NCReportXMLReader*);
    ...
    virtual void updateValue(NCReportEvaluator *evaluator);
